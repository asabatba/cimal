...
export function expandBounds(bounds: GeoBounds, latPadding: number, lonPadding: number): GeoBounds {
  return {
...
export function getBoundsCenter(bounds: GeoBounds): { lat: number; lon: number } {
  return {
...
export function metersPerDegreeAtLatitude(latitude: number): { lat: number; lon: number } {
  const latRad = toRadians(latitude);
  const metersPerDegreeLat = 111_132.92 -
    559.82 * Math.cos(2 * latRad) +
    1.175 * Math.cos(4 * latRad);
  const metersPerDegreeLon = 111_412.84 * Math.cos(latRad) -
