...
function tileId(tile: TileIndex): string {
  return `Copernicus_DSM_COG_${COPERNICUS_RESOLUTION_ARC_SECONDS}_${
    formatLatitudePart(tile.southLat)
...
function computeRasterWindow(
  ...
  const x1 = clamp(
    Math.floor(
      ((overlap.minLon - imageBounds.minLon) / (imageBounds.maxLon - imageBounds.minLon)) *
        imageWidth,
    ),
    0,
    imageWidth - 1,
  ...
  const x2 = clamp(
    Math.ceil(
      ((overlap.maxLon - imageBounds.minLon) / (imageBounds.maxLon - imageBounds.minLon)) *
        imageWidth,
    ),
    x1 + 1,
    imageWidth,
  ...
  const y1 = clamp(
    Math.floor(
      ((imageBounds.maxLat - overlap.maxLat) / (imageBounds.maxLat - imageBounds.minLat)) *
        imageHeight,
    ),
    0,
    imageHeight - 1,
  ...
  const y2 = clamp(
    Math.ceil(
      ((imageBounds.maxLat - overlap.minLat) / (imageBounds.maxLat - imageBounds.minLat)) *
        imageHeight,
    ),
    y1 + 1,
    imageHeight,
...
async function fetchArrayBuffer(url: string): Promise<ArrayBuffer> {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch Copernicus tile ${url}: ${response.status} ${response.statusText}`);
    }
...
function resolveElevation(value: number, noDataValue: number | null): number | null {
...
export function sampleTileElevation(tile: LoadedTile, lat: number, lon: number): number | null {
  ...
  const x = ((lon - bounds.minLon) / (bounds.maxLon - bounds.minLon)) * (width - 1);
  ...
  const samples = [topLeft, topRight, bottomLeft, bottomRight].filter((value): value is number =>
    value != null
  );
  if (samples.length === 0) {
    return null;
...
export function sampleElevation(tiles: LoadedTile[], lat: number, lon: number): number | null {
